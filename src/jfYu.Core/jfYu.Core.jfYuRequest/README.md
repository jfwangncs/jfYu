...
services.AddJfYuHttpRequest(q=>q.LoggingFields= jfYu.Core.jfYuRequest.Enum.JfYuLoggingFields.None); 
//Register with logging enabled and identity filters
services.AddJfYuHttpRequest(q=>{ q.LoggingFields = jfYu.Core.jfYuRequest.Enum.JfYuLoggingFields.All;q.RequestFilter = x => x; q.ResponseFilter = x => x; });
//Register custom HttpClient with cookie handling disabled
services.AddJfYuHttpClient(() => { return new HttpClientHandler() { UseCookies = false }; }, q => { q.LoggingFields = jfYu.Core.jfYuRequest.Enum.JfYuLoggingFields.All; q.RequestFilter = x => x; q.ResponseFilter = x => x; });
